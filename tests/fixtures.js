...
export function makeResult(overrides = {}) {
  return {
    ...
    verdict: 'BUY SETUP',
    verdictColor: 'green',
    verdictSummary: 'small candles, buyers absorbing dips, calm volume, uptrend intact',
    signals: {
      candleSize: makeSignal({ label: 'Small candles', pass: true }),
      candleOverlap: makeSignal({ label: 'Buyers absorbing dip', pass: true, value: '3/3 red candles recovered by a later green close', detail: "green candle closes above red candle's open (want ≥50% of red candles recovered)" }),
...
export function makeNotYetResult(overrides = {}) {
  return makeResult({
    ...
    verdict: 'NOT YET',
    verdictColor: 'yellow',
    verdictSummary: 'buyers not yet absorbing the dip — needs more time',
    signals: {
      candleSize: makeSignal({ pass: true }),
      candleOverlap: makeSignal({ label: 'Buyers absorbing dip', pass: false, value: '0/3 red candles recovered by a later green close', detail: "green candle closes above red candle's open (want ≥50% of red candles recovered)" }),
